...
function updateCaption (hideIndexes, fields) {
  const captionContainer = document.getElementById('vocabuilder-caption')
...
function updateCaptionAndLoadSnapshot ({hideIndexes, fields, showImage, snapshotFilePath}) {
  const captionContainer = document.getElementById('vocabuilder-caption')
  const tts = captionContainer.getElementsByTagName('tt')
  let index = 0
  for (const tt of tts) {
  ...
  const element = document.getElementById('res')
  if (element) {
    const visibility = showImage ? '' : 'hidden'
  ...
  async listen () {
    this.server = net.createServer(socket => {
      socket.on('data', async data => {
        ...
        if (getConfig('useSnapshot')) {
          const snapshotFilePath = this.getSnapshotFilePath(headWord)
          if (fs.existsSync(snapshotFilePath)) {
            snapshotFound = true
            const title = await this.driver.getTitle()
            canReusePage = title === 'Vocabuilder Shapshot Page'
            if (canReusePage) {
              if (getConfig('showCaption')) {
                await this.driver.executeScript(updateCaptionAndLoadSnapshot, {
                  hideIndexes: this.getHideIndexes(),
                  fields: fields,
                  showImage: this.showImage,
                  snapshotFilePath: snapshotFilePath
                })
                // await this.driver.executeScript(updateCaption, this.getHideIndexes(), fields)
              }
            } else {
              await this.driver.get('file://' + PLACE_HOLDER_HTML_PATH)
              await this.driver.executeScript(browserLoadSnapshotImage, {
                showImage: this.showImage,
                snapshotFilePath: snapshotFilePath
              })
            }
        ...
        if (!canReusePage && getConfig('showCaption')) {
